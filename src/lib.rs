...
impl Fernet {
    ...
    fn _encrypt_from_parts(&self, data: &[u8], current_time: u64, iv: &[u8]) -> String {
        ...
        result.push(0x80);
        result.write_u64::<byteorder::BigEndian>(current_time).unwrap();
...
mod tests {
    ...
    fn test_generate_vectors() {
        ...
        let vectors: Vec<GenerateVector> = serde_json::from_str(include_str!("generate.json")).unwrap();

        for v in vectors {
            let f = Fernet::new(v.secret);
            let token = f._encrypt_from_parts(v.src.as_bytes(), chrono::DateTime::parse_from_rfc3339(v.now).unwrap().timestamp() as u64, &v.iv);
    ...
    fn test_verify_vectors() {
        ...
        for v in vectors {
            let f = Fernet::new(v.secret);
            let decrypted = f._decrypt_at_time(v.token, Some(v.ttl_sec), chrono::DateTime::parse_from_rfc3339(v.now).unwrap().timestamp() as u64);
    ...
    fn test_invalid_vectors() {
        // TODO: the vector file shouldn't be in this directory
        let vectors: Vec<InvalidVector> = serde_json::from_str(include_str!("invalid.json")).unwrap();
    ...
    fn test_invalid() {
        ...
        assert_eq!(f.decrypt(&base64::encode_config(b"\x81", base64::URL_SAFE)), Err(DecryptionError));
        // Timestamp too short
        assert_eq!(f.decrypt(&base64::encode_config(b"\x80\x00\x00\x00", base64::URL_SAFE)), Err(DecryptionError));
        // Invalid base64
